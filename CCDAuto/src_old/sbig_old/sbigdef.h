...
//
// 31/03/2002 adapted for Linux character device
// by Jan Soldan, Jan_Soldan@yahoo.com
//========================================================================
...
//========================================================================
#define LPT_DRIVER_VERSION_BCD 0x0106
#define LPT_DRIVER_VERSION_STR "modsbiglpt.o Ver 1.06"

#define USB_DRIVER_VERSION_BCD 0x0106
#define USB_DRIVER_VERSION_STR "modsbigusb.o Ver 1.06"
...
// SBIG: FULL_CLEAR_TIME  6, ie. 330 ms// ticks before use full clear
#define FULL_CLEAR_TIME   33           //  we use 330/10 = 33 ticks

// SBIG: NIBBLE_TIMEOUT = 5 ticks between chars received from camera,
// ie. 5 * 55 = 275 ms
#define NIBBLE_TIMEOUT    28	

// SBIG: IDLE_STATE_DELAY  3, time to force idle at start of packet,
// under linux we use  mdelay(IDLE_STATE_DELAY) with 3 * 55 = 165 ms
#define IDLE_STATE_DELAY  165

// SBIG: CONVERSION_DELAY = 500  queries (approx. 1us each) before
// A/D must signal not busy, under linux we use 500
#define CONVERSION_DELAY  500

// SBIG: VCLOCK_DELAY = 10, delays between vertical clocks on the
// imaging CCD , under linux we use 55
#define VCLOCK_DELAY      55		

// SBIG: ST1K_VCLOCK_X = 10, multiplier for VClock an ST1K,
// under linux we use ?
#define ST1K_VCLOCK_X     10
...
typedef struct{
 ...
 //void           (*callback)(LinuxCallbackParams);
}GrabImageParams;
//------------------------------------------------------------------------
typedef struct{
 unsigned short ccd;          // CCD_REQUEST
