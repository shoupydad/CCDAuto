...
extern CCameraIO *apCam;
extern "C" int CancelCurrentAcquire;
extern "C" void RunMainIteration(float seconds);
extern "C" void ShowImageProgress(char *message, int percent);
...
bool CCameraIO::FilterHome()
{
  ...
  if ( m_HighPriority ) {	// Store current process class and priority
...
bool CCameraIO::Expose( double Duration, bool Light )
{
  ...
  if ( m_TDI ) { // row_offset is the drift time in millisecs when in TDI mode
  ...
  if ( m_TDI ) {
    ...
    if ( val & RegBit_ShutterOverride )
      ...
      m_Shutter = Light;
    m_Shutter_Last_Exposure = m_Shutter;
...
bool CCameraIO::GetImage( unsigned short* pImageData, short& xSize,
			  short& ySize )
{
  int i;
  ...
  if ( m_WaitingforImage ) { // In case application did not poll read_Status
    m_WaitingforImage = false;
  ...
  if ( pImageData != NULL ) {
    ...
    if ( i == m_ExposureSkipR ) {  // We have skipped all the lines
      if (m_Shutter_Last_Exposure)
	...
	if ( ReadLine( SkipPixels, XPixels, pLineBuffer ) )
	  break;
	pLineBuffer += XPixels;
	percent = (int) (100.0 * ((float)  i) / YPixels);
	if (m_Shutter_Last_Exposure)
...
bool CCameraIO::Snap( double Duration, bool Light, unsigned short* pImageData, short& xSize, short& ySize )
{
	...
	while ( true )
	{
		...
		if ( clock() > StopTime )
		  return false;	// Timed out, no image available
		//	Sleep( 220 );	// dont bog down the CPU while polling
		RunMainIteration(0.2); // let gnome run.
	}
...
int InitCam( char* cfgname )
{
  ...
  if ( ret == 0 ) {  // We can now access the cam objects members
    apCam->Flush();	// Start the camera flushing
  } else {           // Got an error
    switch ( ret ) {
...
unsigned short hextoi(char *instr)
{
  ...
  if ( n > 1 ) {	// Look for hex format e.g. 8Fh, A3H or 0x5D
...
//       printf("*** FATAL - ISA interface not supported!\n");
//       exit(-1);
//     } else if ( strcasecmp( "ppi", retbuf ) == 0 ) {
//       printf("*** DEBUG - creating ccameraio object\n");
//       apCam = new CCameraIO();
//     } else if ( strcasecmp( "pci", retbuf ) == 0 ) {
//       printf("*** Fatal - PCI interface not supported!\n");
//       exit(-1);
//     }
  ...
  // Commented out because we use driver so don't need address?

//   if ( BaseAddress == -1 ) {
//     if (CfgGet (inifp, "system", "base", retbuf, sizeof(retbuf), &plen))
//       apCam->m_BaseAddress = hextoi(retbuf) & 0xFFF;
//     else {
//       fclose( inifp );
//       delete apCam;
//       apCam = NULL;
...
//       apCam->m_BaseAddress = BaseAddress & 0xFFF;
  ...
  apCam->Reset();	// Read in command register to set shadow register known state

  if (CfgGet (inifp, "system", "cable", retbuf, sizeof(retbuf), &plen)) {
    if (!strcasecmp("LONG",retbuf))
      apCam->write_LongCable( true );
    else if ( !strcasecmp("SHORT",retbuf) )
      apCam->write_LongCable( false );
  } else
    apCam->write_LongCable( false );	// default
  
  if ( !apCam->read_Present() ) {
    delete apCam;
    apCam = NULL;
    fclose( inifp );
    return CCD_OPEN_LOOPTST;
  ...
  apCam->write_UseTrigger( false );
  apCam->write_ForceShutterOpen( false );

  if (CfgGet (inifp, "system", "high_priority", retbuf, sizeof(retbuf), &plen)) {
    if (!strcasecmp("ON",retbuf) || !strcasecmp("TRUE",retbuf) || !strcasecmp("1",retbuf)) {
      apCam->m_HighPriority = true;
    } else if (!strcasecmp("OFF",retbuf) || !strcasecmp("FALSE",retbuf) ||
	       !strcasecmp("0",retbuf)) {
      apCam->m_HighPriority = false;
    }
  }
  ...
  if (CfgGet (inifp, "system", "data_bits", retbuf, sizeof(retbuf), &plen)) {
    short val = hextoi( retbuf );
    if ( val >= 8 && val <= 18 )
      apCam->m_DataBits = val;
  }

  if (CfgGet (inifp, "system", "sensor", retbuf, sizeof(retbuf), &plen)) {
    if ( strcasecmp( "ccd", retbuf ) == 0 ) {
      apCam->m_SensorType = Camera_SensorType_CCD;
    } else if ( strcasecmp( "cmos", retbuf ) == 0 ) {
      apCam->m_SensorType = Camera_SensorType_CMOS;
    }
  }

  if (CfgGet (inifp, "system", "mode", retbuf, sizeof(retbuf), &plen)) {
    unsigned short val = hextoi(retbuf) & 0xF;
    apCam->write_Mode( val );
  } else
    apCam->write_Mode( 0 ); // default

  if (CfgGet (inifp, "system", "test", retbuf, sizeof(retbuf), &plen)) {
    unsigned short val = hextoi(retbuf) & 0xF;
    apCam->write_TestBits( val );
  } else
    apCam->write_TestBits( 0 );	 //default

  if (CfgGet (inifp, "system", "test2", retbuf, sizeof(retbuf), &plen)) {
    unsigned short val = hextoi(retbuf) & 0xF;
    apCam->write_Test2Bits( val );
  } else
    apCam->write_Test2Bits( 0 );	// default

  apCam->write_FastReadout( false );	//default

  if (CfgGet (inifp, "system", "shutter_speed", retbuf, sizeof(retbuf), &plen)) {
    if (!strcasecmp("normal",retbuf)) {
      apCam->m_FastShutter = false;
      apCam->m_MaxExposure = 10485.75;
      apCam->m_MinExposure = 0.01;
    } else if (!strcasecmp("fast",retbuf)) {
      apCam->m_FastShutter = true;
      apCam->m_MaxExposure = 1048.575;
      apCam->m_MinExposure = 0.001;
  ...
  if (CfgGet (inifp, "system", "shutter_bits", retbuf, sizeof(retbuf), &plen)) {
    unsigned short val = hextoi(retbuf);
    apCam->m_FastShutterBits_Mode = val & 0x0F;
    apCam->m_FastShutterBits_Test = ( val & 0xF0 ) >> 4;
  }
  ...
  if (CfgGet (inifp, "system", "guider_relays", retbuf, sizeof(retbuf), &plen)) {
    if (!strcasecmp("ON",retbuf) || !strcasecmp("TRUE",retbuf) || !strcasecmp("1",retbuf)) {
      apCam->m_GuiderRelays = true;
    } else if (!strcasecmp("OFF",retbuf) || !strcasecmp("FALSE",retbuf) || !strcasecmp("0",retbuf)) {
      apCam->m_GuiderRelays = false;
    }
  }
  ...
  if (CfgGet (inifp, "geometry", "imgcols", retbuf, sizeof(retbuf), &plen)) {
    short val = hextoi(retbuf);
    if ( val >= 1 && val <= MAXCOLUMNS ) 
      apCam->m_ImgColumns = val;
  } else
    apCam->m_ImgColumns = apCam->m_Columns - apCam->m_BIC - apCam->m_SkipC;
  ...
  if (CfgGet (inifp, "geometry", "imgrows", retbuf, sizeof(retbuf), &plen)) {
    ...
    apCam->m_ImgRows = apCam->m_Rows - apCam->m_BIR - apCam->m_SkipR;
  ...
  apCam->m_NumX = apCam->m_ImgColumns;
  apCam->m_NumY = apCam->m_ImgRows;

  // Temperature

  if (CfgGet (inifp, "temp", "control", retbuf, sizeof(retbuf), &plen)) {
    if (!strcasecmp("ON",retbuf) || !strcasecmp("TRUE",retbuf) || !strcasecmp("1",retbuf)) {
      apCam->m_TempControl = true;
    } else if (!strcasecmp("OFF",retbuf) || !strcasecmp("FALSE",retbuf) ||
	       !strcasecmp("0",retbuf)) {
      apCam->m_TempControl = false;
    }
  }
