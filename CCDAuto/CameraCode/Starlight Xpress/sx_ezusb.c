/***************************************************************************\
    ...
    Copyright (C) 2002, 2003 David Schmenk
...
#define CCD_VCLK()                                                          \
    do {                                                                    \
        PORTA_OUT = (ccd_field_xor_mask == HX9_FIELD_MAGIC) ? 0xDD : 0xBE;  \
        PORTA_OUT = ccd_rdy;                                                \
        udelay(5);                                                          \
    } while (0)
#define CCD_HCLK()                                                          \
...
code byte sx_ccd_hfront_porch[]   = {HX5_HFRONT_PORCH, HX9_HFRONT_PORCH, MX5_HFRONT_PORCH, MX7_HFRONT_PORCH, MX9_HFRONT_PORCH};
code byte sx_ccd_hback_porch[]    = {HX5_HBACK_PORCH,  HX9_HBACK_PORCH,  MX5_HBACK_PORCH,  MX7_HBACK_PORCH,  MX9_HBACK_PORCH};
code byte sx_ccd_vfront_porch[]   = {HX5_VFRONT_PORCH, HX9_VFRONT_PORCH, MX5_VFRONT_PORCH, MX7_VFRONT_PORCH, MX9_VFRONT_PORCH};
code byte sx_ccd_vback_porch[]    = {HX5_VBACK_PORCH,  HX9_VBACK_PORCH,  MX5_VBACK_PORCH,  MX7_VBACK_PORCH,  MX9_VBACK_PORCH};
code word sx_ccd_width[]          = {HX5_WIDTH,        HX9_WIDTH,        MX5_WIDTH,        MX7_WIDTH,        MX9_WIDTH};
code word sx_ccd_height[]         = {HX5_HEIGHT,       HX9_HEIGHT,       MX5_HEIGHT,       MX7_HEIGHT,       MX9_HEIGHT};
code word sx_ccd_pix_width[]      = {HX5_PIX_WIDTH,    HX9_PIX_WIDTH,    MX5_PIX_WIDTH,    MX7_PIX_WIDTH,    MX9_PIX_WIDTH};
code word sx_ccd_pix_height[]     = {HX5_PIX_HEIGHT,   HX9_PIX_HEIGHT,   MX5_PIX_HEIGHT,   MX7_PIX_HEIGHT,   MX9_PIX_HEIGHT};
 ...
 */
xdata at 0x7C80 byte ex_usb_dbg[USB_BUF_SIZE];
...
code byte usb_device_descriptor[] =
{
    18,         // Length
    1,          // Type
    ...
    USB_BUF_SIZE,
    0x44, 0x44, // Vendor ID
    0x20, 0x42, // Product ID
    0x00, 0x00, // Version
    1, 2, 0,    // Manufacturer, Product, Serial # string ID
    1           // Number of configurations
};
code byte usb_config_descriptor[] =
{
    9,          // Length
    2,          // Type
...
code byte usb_blockio_interface_descriptor[] =
{
    9,          // Length
    4,          // Type
    1,          // ID
...
code byte usb_blockio_in_endpoint_descriptor[] =
{
    7,          // Length
    5,          // Type
    0x82,       // Address = IN2
...
code byte usb_blockio_out_endpoint_descriptor[] =
{
    7,          // Length
    5,          // Type
    0x02,       // Address = OUT2
 ...
 * WakeUp ISR.
 */
void wakeup_isr(void) interrupt 6 using 1
{
...
void ccd_set_default_params(void)
{
    ...
    ccd_color_matrix = ccd_color ? (SXUSB_COLOR_MATRIX_2X2 | SXUSB_COLOR_MATRIX_ALT_ODD | (ccd_vfront_porch & 1 ? 0x5B6 : 0x6B5))
                                 : SXUSB_COLOR_MONOCHROME;
}
signed char sx_request(xdata byte *param_buf, xdata byte *in_buf)
{
    signed char count = 0;
    if (usb_configured)
        switch (usb_setup_req[USB_REQ])
        {
            ...
            case SXUSB_READ_PIXELS_DELAYED:
                ...
                ccd_load_pending = 0;
                ccd_timer        = *((xdata dword *)&param_buf[10]);
                /*
            ...
            case SXUSB_READ_PIXELS:
                ...
                ccd_read_xoffset = *((xdata word *)&param_buf[0]);
                ccd_read_yoffset = *((xdata word *)&param_buf[2]);
                ccd_read_width   = *((xdata word *)&param_buf[4]);
                ccd_read_height  = *((xdata word *)&param_buf[6]);
            ...
            case SXUSB_SET_TIMER:
                ccd_timer = *((xdata dword *)param_buf);
                if (ccd_timer)
                {
                    TL0 = TIMER0_RELOAD;
                    TH0 = TIMER0_RELOAD >> 8;
            ...
            case SXUSB_SET_CCD:
                ...
                ccd_width          = *((xdata word *)&param_buf[2]);
                ccd_vfront_porch   = param_buf[4];
                ccd_vback_porch    = param_buf[5];
                ccd_height         = *((xdata word *)&param_buf[6]);
                ccd_pixel_width    = *((xdata word *)&param_buf[8]);
                ccd_pixel_height   = *((xdata word *)&param_buf[10]);
                ccd_color_matrix   = *((xdata word *)&param_buf[12]);
            ...
            default:
                ...
                {
                    in_buf[0] = FIRMWARE_MINOR_VERSION;
                    in_buf[1] = FIRMWARE_MINOR_VERSION >> 8;
                    in_buf[2] = FIRMWARE_MAJOR_VERSION;
                    in_buf[3] = FIRMWARE_MAJOR_VERSION >> 8;
                    count     = 4;
                }
                else
                    count = -1;
