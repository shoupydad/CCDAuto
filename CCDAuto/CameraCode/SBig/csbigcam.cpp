...
/*
	...
	Returns full info on model camera
	that has been linked to.

*/

PAR_ERROR CSBIGCam::GetCCDInfo(GetCCDInfoParams *gcip, void *gcir)
{
  PAR_ERROR status;

  status = SBIGUnivDrvCommand(CC_GET_CCD_INFO, gcip, gcir);
  return status;
}
...
*/
PAR_ERROR CSBIGCam::QueryTemperatureStatus(MY_LOGICAL *enabled,
					   double *ccdTemp, double *airTemp,
					   double *setpointTemp,
					   double *percentTE)
{
	...
	if ( CheckLink() ) {
    	if ( SBIGUnivDrvCommand(CC_QUERY_TEMPERATURE_STATUS, NULL, &qtsr) == CE_NO_ERROR )
    	{
    		...
    		*airTemp = ADToDegreesC(qtsr.ambientThermistor, TRUE);
...
{
	double setpointTemp, percentTE, airTemp;
	MY_LOGICAL teEnabled;

	return QueryTemperatureStatus(&teEnabled, ccdTemp, &airTemp,
